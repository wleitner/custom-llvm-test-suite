...
awk -F' ' '{if ($2 ~ /^task-clock/ || $3 ~ /^task-clock/) {gsub(/,/,"",$1);print "user",$1/1000;} else if ($2 == "seconds" && $4 == "elapsed") {print "real",$1;} else if ($2 ~ /^instructions/) {gsub(/,/,"",$1);print "instructions",$1} else if ($2 ~ /^cycles/) {gsub(/,/,"",$1);print "cycles",$1}}' $PERFSTAT >> $REPORT
