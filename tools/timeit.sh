...
if [ "$OUTPUT" = "$ERRPUT" ]; then
  ...
  perf stat --hybrid-merge -o $PERFSTAT $@ < $INPUT >& $OUTPUT
else
  perf stat --hybrid-merge -o $PERFSTAT $@ < $INPUT > $OUTPUT 2> $ERRPUT
fi
...
awk -F' ' '{if ($2 ~ /^cpu-clock/ || $3 ~ /^cpu-clock/) {gsub(/,/,"",$1);print "user",$1/1000;} else if ($2 == "seconds" && $4 == "elapsed") {print "real",$1;} else if ($2 ~ /^instructions/) {gsub(/,/,"",$1);print "instructions",$1} else if ($2 ~ /^cycles/) {gsub(/,/,"",$1);print "cycles",$1}}' $PERFSTAT >> $REPORT
